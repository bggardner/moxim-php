<?php
  ...
  require_once 'MySQLPDOService.php';
  
  use \RuntimeException;
  
  /* This class provides error checking to the extended service. */
  
  class BaseService extends MySQLPDOService
  {
    ...
    public function deleteModule($id)
    {
      if (is_null($id))
      {
        throw new RuntimeException('Module id is reuired for deletiion.');
      }
      if (!is_int($id) || $id < 1)
      {
        throw new RuntimeException('Module id must be a positive integer, '.gettype($id).'('.htmlspecialchars($id).') given.');
      }
      return parent::deleteModule($id);
    }
  }
